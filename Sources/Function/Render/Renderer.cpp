...
namespace FOCUS
{
	...
	void Renderer::buildAndSubmit(std::vector<std::shared_ptr<RenderResource>>* renderlist, std::vector<DRHI::DynamicCommandBuffer>* commandBuffers, DRHI::DynamicCommandPool* commandPool)
	{
		...
		for (auto p : _submitRenderlist)
		{
			...
			p->_material->_filteredImageView = &_environmentMap->_material->_textureImageView;//&_filteredImageView;
			p->_material->_filteredImageSampler = &_environmentMap->_material->_textureSampler;//&_filteredImageSampler;
	...
	void Renderer::prefilterEnvironmentMap()
	{
		...
		// create image
		{
			DRHI::DynamicImageCreateInfo imageCI{};
			imageCI.format = format.FORMAT_B8G8R8A8_SRGB;
			imageCI.extent.width = _filteredImageWidth;
			imageCI.extent.height = _filteredImageHeight;
			imageCI.extent.depth = 1;
			imageCI.mipLevels = 1;
			...
			viewInfo.format = format.FORMAT_B8G8R8A8_SRGB;
			viewInfo.image = _filteredImage;
			viewInfo.type = viewType.IMAGE_VIEW_TYPE_2D;
			viewInfo.subresourceRange.aspectMask = aspect.IMAGE_ASPECT_COLOR_BIT;
			viewInfo.subresourceRange.baseArrayLayer = 0;
			viewInfo.subresourceRange.baseMipLevel = 0;
		...
		DRHI::DynamicImageCreateInfo ici{};
		ici.format = format.FORMAT_B8G8R8A8_SRGB;
		ici.extent.width = _filteredImageWidth;
		ici.extent.height = _filteredImageHeight;
		ici.extent.depth = 1;
		ici.mipLevels = 1;
		ici.arrayLayers = 1;
		ici.samples = samples.SAMPLE_COUNT_1_BIT;
		...
		fcreateInfo.attachmentCount = 1;
		fcreateInfo.pAttachments = &offscreenImageView;
		fcreateInfo.width = _filteredImageWidth;
		fcreateInfo.height = _filteredImageHeight;
		fcreateInfo.layers = 1;
		...
		rbinfo.renderPass = renderPass;
		rbinfo.framebuffer = framebuffer;
		rbinfo.renderArea.extent.width = _filteredImageWidth;
		rbinfo.renderArea.extent.height = _filteredImageHeight;
		...
		DRHI::DynamicViewport viewPort{};
		viewPort.width = _filteredImageWidth;
		viewPort.height = _filteredImageHeight;
		viewPort.minDepth = 0.0f;
		viewPort.maxDepth = 1.0f;
		viewPort.x = 0;
		viewPort.y = 0;
		...
		DRHI::DynamicRect2D scissor{};
		scissor.extent.width = _filteredImageWidth;
		scissor.extent.height = _filteredImageHeight;
		scissor.offset.x = 0;
		scissor.offset.y = 0;
		_rhiContext->cmdSetScissor(commandBuffer, 0, 1, scissor);
		...
		pushBlock.mvp = perspective((float)(PI / 2.0), 1.0f, 0.1f, 512.f);
	...
	void Renderer::precomputeIrradianceMap()
	{
		...
		// create irradiance image
		{
			...
			// create irradiance image view
			DRHI::DynamicImageViewCreateInfo vci{};
			vci.type = viewType.IMAGE_VIEW_TYPE_2D;
			vci.format = format.FORMAT_B8G8R8A8_SRGB;
			vci.image = _irradianceImage;
			vci.subresourceRange.aspectMask = aspect.IMAGE_ASPECT_COLOR_BIT;
		...
		ici.format = format.FORMAT_B8G8R8A8_SRGB;
		ici.extent.width = _filteredImageWidth;
		ici.extent.height = _filteredImageWidth;
		ici.extent.depth = 1;
		ici.mipLevels = 1;
		ici.arrayLayers = 1;
		ici.samples = samples.SAMPLE_COUNT_1_BIT;
		...
		binfo.framebuffer = framebuffer;
		binfo.renderPass = renderPass;
		binfo.renderArea.extent.width = _filteredImageWidth;
		binfo.renderArea.extent.height = _filteredImageHeight;
		binfo.renderArea.offset.x = 0;
		binfo.renderArea.offset.y = 0;
		...
		// start render pass
		viewport.width = static_cast<float>(_filteredImageWidth);
		viewport.height = static_cast<float>(_filteredImageHeight);
		_rhiContext->cmdSetViewport(commandBuffer, 0, 1, viewport);

		_rhiContext->beginRenderPass(&commandBuffer, &binfo, content.SUBPASS_CONTENTS_INLINE);

		pushBlock.mvp = perspective((float)(PI / 2.0), 1.0f, 0.1f, 512.0f);
